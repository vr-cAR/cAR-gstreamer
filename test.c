...
typedef struct _CustomData {
    k4a_device_t device;
    GstElement *pipeline;
    GstElement *app_source, *jpeg_dec, *queue, *omx_enc, *video_convert, *video_scale, *video_out;
    GstElement *fakesink, *app_sink, *rtp_sink, *udp_sink;
    GstElement *rtp_pay;
...
int main(int argc, char* argv[]){
    ...
    // Flag for jpegdec
    // g_object_set(data.jpeg_dec, "idct-method", 2, NULL);
    ...
    gst_bin_add_many(GST_BIN(data.pipeline), data.app_source, data.jpeg_dec, data.omx_enc, data.rtp_pay, data.udp_sink, NULL);
    if (gst_element_link_many(data.app_source, data.jpeg_dec, data.omx_enc, data.rtp_pay, data.udp_sink, NULL) != TRUE) {
